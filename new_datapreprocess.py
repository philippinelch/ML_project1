...
def split_and_balance_data(x, y, test_size=0.2, balance_method="oversample", random_seed=42):
    """
...
def one_hot_encode_with_categories(data, categorical_columns, categories_per_column):
    """
    ...
    for col_idx in range(data.shape[1]):
        if col_idx in categorical_columns:
            # Use predefined categories for this column
...
def Preprocess_Data(
    file_path, x_train, y_train, test_size=0.2, balance_method="oversample",
    ...
    x_train_balanced, x_test, y_train_balanced, y_test = split_and_balance_data(
        x_train, y_train, test_size, balance_method
    )
    ...
    all_data = np.vstack([x_train_balanced, x_test])
    ...
    x_test_encoded, binary_test_indices = one_hot_encode_with_categories(x_test, categorical_columns_indices, categories_per_column)
