...
def correlation_analysis(headers, x_data, y_data, low_threshold=0.05, high_threshold=0.9):
    ...
    or are highly correlated with other features, leading to potential rank deficiency.

    Args:
        headers (list): The list of feature names.
        x_data (np.array): The feature dataset with features as columns.
        y_data (np.array): The outcome variable (labels).
        low_threshold (float): Minimum Pearson correlation threshold with y_data.
        high_threshold (float): Maximum Pearson correlation threshold for inter-feature correlation.

    Returns:
        tuple: A tuple containing:
            - valid_columns (list): Indices of features that are kept.
            - filtered_data (np.array): Filtered dataset with only valid columns.
    """
    valid_columns = []
    removed_columns = []

    # Step 1: Correlation with y_data
    ...
    # Step 2: Inter-feature correlation for remaining features
    filtered_data = x_data[:, valid_columns]
    to_remove = set()
    for i in range(filtered_data.shape[1]):
        for j in range(i + 1, filtered_data.shape[1]):
            if abs(np.corrcoef(filtered_data[:, i], filtered_data[:, j])[0, 1]) > high_threshold:
                # Remove the j-th column in the pair to avoid multicollinearity
                to_remove.add(valid_columns[j])

    # Final set of valid columns after removing highly correlated features
    valid_columns = [col for col in valid_columns if col not in to_remove]
    removed_columns.extend(list(to_remove))

    # Log removed features
    log_and_print_removals(headers, removed_columns, "3: Correlation Analysis", "Low or high correlation")

    # Filter valid columns in the final dataset
    filtered_data = x_data[:, valid_columns]
    return valid_columns, filtered_data
